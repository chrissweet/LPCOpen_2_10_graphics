...
/* This bit is in the SCS (System Controls and Status register).  It sets the
   polarity of the SD_PWR output. When set, the SD_PWR output is active
   high. When clear, the SD_PWR output is active low. */
#define SYSCTL_MCIPWRAL_BITMASK (((uint32_t) 1) << 3)
...
/**
 * @brief	Set the power control (SD_PWR) output polarity
 * @param	pSDC	: Pointer to SDC register block
 * @param	polarity: 1 - Active high, 0 - Active low
 * @return	Nothing
 */
void Chip_SDC_PowerControlPolarity(LPC_SDC_T *pSDC, bool polarity);
